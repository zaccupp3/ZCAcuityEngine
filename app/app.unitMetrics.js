...
// ---------------------------------------------------------
// UNIT METRICS — Chart-first MVP (hardened)
//
// HARD GUARANTEES:
// ✅ Only one UnitMetrics runtime (singleton guard)
// ✅ Latest request wins (requestId guard)
// ✅ Chart shell never stays missing (auto-repair observer)
// ✅ Analytics-first, snapshot fallback
//
// NOTE (compat):
// This file still uses the existing DOM ids (pulseSummary, btnLoadPulse, etc.)
// so you do NOT need to rename HTML yet. Labels/behavior are “Unit Metrics”.
// ---------------------------------------------------------
...
(function () {
  ...
  // Keep old DOM id (#pulseStatusMsg) but expose “metrics” terminology
  ...
  function ensureChartShell() {
    ...
    // Keep DOM ids the same; just change the visible label text
    container.innerHTML = `
      <div id="pulseChartsWrap" style="display:flex;flex-direction:column;gap:12px;">
        <div class="staff-card">
          <div class="staff-card-header">Unit Metrics Summary</div>
  ...
  function renderDashboard(rowsRaw, reqId) {
    if (reqId !== __metricsReqSeq) return;
  ...
  function installRepairObserver() {
    ...
    if (!root || root.__metricsObserverInstalled) return;
    root.__metricsObserverInstalled = true;
    ...
    const obs = new MutationObserver(() => {
      if (!chartCanvasesExist() && __lastGoodRowsRaw && __lastGoodRowsRaw.length) {
        ...
        setTimeout(() => {
          const reqId = __lastGoodReqId || __metricsReqSeq;
  ...
  async function loadUnitMetrics() {
    ...
    if (!unitId) {
      setMetricsStatus("No active unit selected.", true);
      renderDashboard([], reqId);
      return;
    }
    if (!sbReady()) {
      setMetricsStatus("Offline/demo mode — Supabase not ready.", true);
      renderDashboard([], reqId);
      return;
    }
    ...
    try {
      ...
      if (a.ok && a.rows.length) {
        setMetricsStatus(`Loaded ${a.rows.length} analytics shift metric row(s).`);
      ...
      if (!s.ok) {
        setMetricsStatus(`Could not load metrics (${s.error?.message || s.error}).`, true);
        renderDashboard([], reqId);
        return;
      }

      setMetricsStatus(`Loaded ${s.rows.length} shift snapshot(s) (fallback).`);
      __lastGoodRowsRaw = s.rows.slice();
      __lastGoodReqId = reqId;
      renderDashboard(s.rows, reqId);
    } catch (e) {
      if (reqId !== __metricsReqSeq) return;
      setMetricsStatus(`Error loading metrics (${String(e)}).`, true);
      renderDashboard([], reqId);
    }
  }
  ...
  function wireMetricsUI() {
    const btn = $("btnLoadPulse");
    if (btn && !btn.__metricsWired) {
      btn.addEventListener("click", () => loadUnitMetrics());
      btn.__metricsWired = true;
    }
    ...
    [fromEl, toEl, shiftEl].forEach(node => {
      if (!node || node.__metricsWired) return;
      node.addEventListener("change", () => loadUnitMetrics());
      node.__metricsWired = true;
    });

    window.unitMetrics = window.unitMetrics || {};
    window.unitMetrics.load = loadUnitMetrics;

    // Back-compat: keep older callers working
  ...
  window.addEventListener("DOMContentLoaded", () => {
    ...
    installRepairObserver();
    wireMetricsUI();
    setMetricsStatus("Choose a range and click “Load Metrics”.");
  });
})();