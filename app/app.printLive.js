/* =========================================================
   ...
   - Prints LIVE (CURRENT) Leadership + RN cards + PCA cards
   - Header style mirrors app.printOncoming.js
========================================================= */
...
(function () {
  ...
  function parseCard(block, kind) {
    // LIVE cards: .assignment-header contains a <strong>name</strong>
    const titleEl =
    ...
    const rows = bodyRows
      .map((tr) => {
        ...
        // LIVE tables: [Room, Level, Acuity Notes]
  ...
  function detectShift() {
    // Prefer your canonical shift state if present
  ...
  function buildPrintHTML(data) {
    ...
    const rnCards = renderCards(data.rnCards, {
      section: "Current RN Assignments",
      kind: "RN",
      headerHtml: topBar,
    });

    const pcaCards = renderCards(data.pcaCards, {
      section: "Current PCA Assignments",
      kind: "PCA",
      headerHtml: topBar,
    });
...
<style>
  ...
  *{ box-sizing: border-box; }
  ...
  td.col-notes{ text-align: center; }
...
<body data-pca-tight="${pcaTight}" data-pca-cards="${pcaCardsMode}">
  ...
  function getOrCreatePrintFrame() {
    let frame = document.getElementById("printFrameLive");
    if (frame) return frame;

    frame = document.createElement("iframe");
    frame.id = "printFrameLive";
  ...
  function openPrintPreview() {
    try {
      // Try common LIVE leadership IDs, but never fail if they don't exist.
      const charge = firstNonEmpty([
        getValueById("currentChargeName"),
        getValueById("chargeName"),
        getValueById("liveChargeName"),
        getValueById("incomingChargeName"), // fallback only
      ]);
      ...
      setTimeout(() => {
        try {
          ...
          console.error("[printLive] iframe print() failed:", e);
  ...
  // Export
  window.printLive = { open: openPrintPreview };
